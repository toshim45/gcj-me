...
const (
	inputPath  string = "/Users/artikow/Downloads/A-small-practice.in"
	outputPath string = "/Users/artikow/Downloads/oversized-pancake-flipper.out"
	Eol        string = "\n"
)
...
func process(b *bufio.Scanner) string {
	var raw = scanText(b)
	var raws = strings.Split(raw, " ")
	var s = raws[0]
	var k, _ = strconv.Atoi(raws[1])
	var maxFlip int = 0
	var firsts, ks, lasts string
	var dashIdx int
	var lenS = len(s)
	...
	for !AllPositive(s) {
		...
		ks = s[dashIdx : dashIdx+k]
		lasts = s[dashIdx+k : lenS]
		ks = Flip(ks)
		s = firsts + ks + lasts
		maxFlip++
	}
...
In Case #1,
you can get all the pancakes happy side up by first flipping the leftmost 3 pancakes,
